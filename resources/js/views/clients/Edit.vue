  ...
  <div class="min-h-screen bg-gray-50">
    <div class="container mx-auto px-4 py-8">
      <div class="max-w-2xl mx-auto">
        ...
        <div v-if="loadingClient" class="bg-white rounded-lg shadow p-6">
          <p class="text-gray-600">Cargando datos del cliente...</p>
        </div>
...
const fetchClient = async () => {
  ...
  console.log('Fetching client for edit with ID:', route.params.id)
  ...
  try {
    ...
    if (response.data.status) {
      ...
      loadError.value = 'Error al cargar los datos del cliente'
    }
  } catch (err) {
    console.error('Error fetching client:', err)
    loadError.value = err.response?.data?.message || 'Ocurrió un error al cargar los datos del cliente'
  } finally {
...
const handleSubmit = async () => {
  ...
  try {
    const response = await axios.put(`/clients/${route.params.id}`, form.value)
    ...
    if (response.data.status) {
      ...
      error.value = response.data.message || 'Error al actualizar el cliente'
    }
  } catch (err) {
    ...
    if (err.response?.data?.errors) {
      ...
      error.value = err.response?.data?.message || 'Ocurrió un error al actualizar el cliente'
    }
  } finally {
    loading.value = false
  }
